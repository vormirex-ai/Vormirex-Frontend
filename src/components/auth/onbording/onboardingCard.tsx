...
const SelectableCard = ({
  ...
  return (
    <motion.button
      onClick={onClick}
      className={cn(
        "relative rounded-2xl border p-8 text-left transition-all duration-300 w-full overflow-hidden group",
        selected
    ...
    >
      ...
      <div className="flex flex-col items-center text-center relative z-10">
        <div
          className="text-4xl mb-4 flex items-center justify-center transition-transform duration-300 group-hover:scale-110"
        ...
        {description?.trim() && (
          <p className="mt-2 text-xs text-textColor group-hover:text-white/80 transition-colors">
            {description}
          </p>
        )}
      </div>
